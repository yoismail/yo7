...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    return new Response(JSON.stringify({
      error: "Couldn't verify that payment right now, please try again in a moment."
    }), {
      status: 200,
      headers: {
        ...cors,
  ...
  if (paymentIntent.status !== "succeeded") {
    return new Response(JSON.stringify({
      error: "That payment hasn't actually succeeded yet."
    }), {
      status: 200,
      headers: {
        ...cors,
  ...
  const { data: result, error: finalizeError } = await db.rpc("finalize_order_from_pending", {
    ...
    p_payment_method_summary: paymentMethodSummary
  }).single();
  if (finalizeError) {
    console.error("finalize_order_from_pending failed:", finalizeError.message);
    return new Response(JSON.stringify({
      error: "Something went wrong finishing your order, please try again."
    }), {
      status: 200,
      headers: {
        ...cors,
