...
async function loadPricingSettings(db) {
  ...
  if (Array.isArray(data.delivery_weight_tiers) && data.delivery_weight_tiers.length) {
    DELIVERY_WEIGHT_TIERS = data.delivery_weight_tiers.map((t)=>({
        maxWeight: t.maxWeight === null ? Infinity : t.maxWeight,
        fee: t.fee
      }));
  }
  ...
  BUNDLES.forEach((b, i)=>{
    if (typeof bundleDiscounts[i] === "number") b.discountPercent = bundleDiscounts[i];
  });
  const comboDiscounts = data.combo_discounts || {};
  COMBOS.forEach((c, i)=>{
    if (typeof comboDiscounts[i] === "number") c.discountPercent = comboDiscounts[i];
  });
}
...
function parseProductCartId(id) {
...
Deno.serve(async (req)=>{
  ...
  if (!secretKey) {
    ...
    return new Response(JSON.stringify({
      error: "Payment isn't configured on the server yet."
    }), {
      status: 200,
      headers: {
        ...cors,
  ...
  if (Math.random() < 0.02) {
    db.rpc("prune_rate_limit_hits").then(({ error })=>{
  ...
  async function priceCart(items, discountCodeRaw) {
    ...
    const { data: productRows } = await db.from("products").select("cat_slug, idx, price, sale_price, stock, weight").in("cat_slug", catSlugs);
    ...
    const { data: customProductRows } = await db.from("custom_products").select("cat_slug, idx, price, sale_price, stock, weight, stock_quantity").in("cat_slug", catSlugs);
    const { data: overrideRows } = await db.from("product_overrides").select("product_key, price, sale_price, stock, weight, weight_variants, stock_quantity").in("product_key", [
      ...productKeys
    ]);
    const base = new Map();
    (productRows ?? []).forEach((r)=>base.set(`${r.cat_slug}::${r.idx}`, r));
    (customProductRows ?? []).forEach((r)=>{
      const key = `${r.cat_slug}::${r.idx}`;
      if (!base.has(key)) base.set(key, r);
    });
    const overrides = new Map();
    (overrideRows ?? []).forEach((r)=>overrides.set(r.product_key, r));
    function trusted(key) {
    ...
    for (const it of items){
      ...
      if (qty <= 0 || typeof it.id !== "string") {
        rejection = "Invalid cart item.";
        break;
      }
      ...
      if (defs) {
        if (!defs[idx]) {
          rejection = `Unknown bundle/combo: ${it.id}`;
          break;
        }
        const def = defs[idx];
        let rawTotal = 0;
      ...
      if (parsed) {
        ...
        if (!t) {
          rejection = `Unknown product: ${it.id}`;
          break;
        }
        ...
        if (t.stock === "out") {
          rejection = "One of the items in your basket just went out of stock, please remove it and try again.";
          break;
        }
        ...
        if (t.stockQuantity !== null) {
          ...
          if (requestedSoFar > t.stockQuantity) {
            rejection = t.stockQuantity <= 0 ? "One of the items in your basket just sold out, please remove it and try again." : `Only ${t.stockQuantity} of one item in your basket ${t.stockQuantity === 1 ? 'is' : 'are'} left in stock, please reduce the quantity.`;
            break;
          }
        }
        let unitPrice;
        ...
        if (parsed.variantLabel !== null) {
          ...
          if (!v) {
            rejection = `Unknown product variant: ${it.id}`;
            break;
          }
        ...
        lines.push({
          ...
          qty,
          weight: weight * qty,
          catSlug: parsed.baseKey.split("::")[0],
          productId: it.id,
          stockKey: parsed.baseKey
        });
        continue;
      }
    ...
    let discountDef = null;
    const discountCode = typeof discountCodeRaw === "string" ? discountCodeRaw.trim() : "";
    let discountError = null;
    if (discountCode) {
      const { data: d } = await db.from("discount_codes").select("*").ilike("code", discountCode).maybeSingle();
      if (!d) discountError = "That code doesn't match any current discount.";
        ...
        if (d.start_date && today < d.start_date) discountError = `That code isn't valid until ${d.start_date}.`;
          ...
          const qualifying = d.qualifying_scope === "category" ? lines.filter((l)=>l.catSlug === d.qualifying_category) : d.qualifying_scope === "product" ? lines.filter((l)=>l.productId.startsWith(d.qualifying_product_id ?? " ")) : lines;
          if (qualifying.length === 0) discountError = "Your basket doesn't contain the items this code applies to.";
            ...
            const { count } = await db.from("discount_redemptions").select("id", {
              count: "exact",
              head: true
            }).eq("discount_id", d.id).eq("user_id", userId);
            if ((count ?? 0) >= d.max_per_customer) discountError = "You've already used this code the maximum number of times.";
          }
          if (!discountError) {
            discountRow = {
              id: d.id,
              max_per_customer: d.max_per_customer
            };
            discountDef = {
              id: d.id,
              code: d.code,
              ...
              buyQty: d.buy_qty,
              getQty: d.get_qty,
              qualifyingScope: d.qualifying_scope,
              qualifyingCategory: d.qualifying_category,
              qualifyingProductId: d.qualifying_product_id
            };
            ...
            if (d.type === "percent") discountAmount = round2(qualifyingSubtotal * (d.value / 100));
              ...
              qualifying.forEach((l)=>{
                for(let n = 0; n < l.qty; n++)units.push(l.unitPrice);
              });
              units.sort((a, b)=>a - b);
              const groupSize = (d.buy_qty ?? 0) + (d.get_qty ?? 0);
              if (groupSize > 0) {
                units.forEach((price, i)=>{
                  if (i % groupSize >= (d.buy_qty ?? 0)) discountAmount += price;
                });
    ...
    let loyaltyPct = null;
    if (userId) {
      const { data: reward } = await db.from("loyalty_rewards").select("pct").eq("user_id", userId).eq("used", false).limit(1).maybeSingle();
      if (reward) {
    ...
    return {
      ...
      discountAmount,
      discountError,
      discountId: discountRow?.id ?? null,
      discountDef,
      freeDelivery,
  ...
  if (amount < 30) {
    return new Response(JSON.stringify({
      error: "Order total is below the minimum payable amount."
    }), {
      status: 200,
      headers: {
        ...cors,
  ...
  const deliveryInfo = body.deliveryInfo && typeof body.deliveryInfo === "object" ? body.deliveryInfo : {};
  ...
  const deliveryAddress = typeof deliveryInfo.address === "string" ? `${deliveryInfo.address}, ${deliveryInfo.town ?? ""} ${deliveryInfo.postcode ?? ""}`.trim() : null;
  try {
